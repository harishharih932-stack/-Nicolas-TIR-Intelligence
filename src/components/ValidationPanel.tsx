    ...
    <Card className="p-6 bg-white border-slate-200 space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-slate-500">Dataset matching logic from scripts/downscale.py</p>
        </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="space-y-3">
          <p className="text-xs font-bold uppercase text-slate-400">2. Simulated Input (100m)</p>
          <div className="aspect-square bg-slate-100 rounded-lg border flex items-center justify-center overflow-hidden">
             {simulatedTir ? <img src={simulatedTir} className="w-full h-full object-cover" /> : <span className="text-[10px] text-slate-300">Run simulation</span>}
          </div>
        </div>
      </div>
